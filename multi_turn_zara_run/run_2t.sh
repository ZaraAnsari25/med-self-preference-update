#!/bin/bash
# Pipeline 1 (multi-turn) — 2t generation + evaluation, fully unattended.
#
# Usage:  bash multi_turn_zara_run/run_2t.sh [N] [T]
#           N = number of scenarios (default 200)
#           T = max tokens per turn  (default 500)
#         e.g. `bash multi_turn_zara_run/run_2t.sh 10`  for a quick 10-scenario test.
...
LOG="multi_turn_zara_run/run_2t_$(date +%Y%m%d_%H%M%S).log"

echo "=== 2t run STARTED $(date) | N=$N T=$T ===" | tee "$LOG"

# --- Stage 1: generation (GPT-5.5 + Claude Sonnet 5) ---
caffeinate -i "$PY" src/generation/generate_conversations.py \
  --num_scenarios "$N" --turns 2 --shuffle --seed 42 \
